...
//    	if(notification.getNotificationContent() == null) return;
		...
		for(WebSocketSession s : sessions) {
            ...
            if(loginMemberNo == notification.getReceiveShopNo()) { //보내는 조건을 까다롭게 만듦
...
//		if(sendMember.getMemberNo() == hire.getShopNo()) return;
		// *****************************************************************
    	...
    	case "hireApply" :
    		...
    		// 알림 내용 가공
    		content = String.format("<b>%s</b>님이 <b>[%s]</b> 공고글에 지원하였습니다.", 
    				sendMember.getMemberName(), hire.getHireTitle());
    		...
    		// 알림 받을 회원 번호 세팅
    		notification.setReceiveShopNo(hire.getShopNo());
    		break;
...
//    				sendMember.getMemberName(), board.getBoardTitle());
//			// 알림 내용 세팅
//    		notification.setNotificationContent(content);
//    		
//    		// 알림 받을 회원 번호 세팅
