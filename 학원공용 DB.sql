...
JOIN WORK_TYPE wt  USING (TYPE_NO)
WHERE RESUME_NO=10;
SELECT * FROM "HIRE"
		WHERE HIRE_DEL_FL='N'
		AND SHOP_NO=100
		ORDER BY HIRE_STATUS,HIRE_NO DESC;
	 ...
	 SELECT * FROM "RESUME"
	 	WHERE RESUME_NO=10;
SELECT A.MEMBER_NO, h.HIRE_NO, C.RESUME_NO, C.RESUME_TITLE,A.APPLY_TITLE,
		A.APPLY_CONTENT, A.APPLY_DATE, h.SHOP_NAME, h.HIRE_TITLE,
		B.MEMBER_NAME, B.MEMBER_PHONE_NUMBER, B.MEMBER_EMAIL
		FROM "APPLY_HIRE" A 
		LEFT JOIN "MEMBER" B ON (A.MEMBER_NO=B.MEMBER_NO)
		LEFT JOIN "RESUME" C ON (A.MEMBER_NO=C.MEMBER_NO)
		LEFT JOIN "HIRE" h ON (A.HIRE_NO=h.HIRE_NO)
	    LEFT JOIN SHOP s ON (h.SHOP_NO = s.SHOP_NO)
		WHERE A.MEMBER_NO = 100
		AND h.HIRE_NO = 110;
