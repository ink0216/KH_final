...
ALTER TABLE RESUME ADD IMG_RENAME VARCHAR2(200);
ALTER TABLE RESUME DROP COLUMN IMAGE;
COMMIT;
CREATE SEQUENCE SEQ_RESUME_EDU_NO NOCACHE;
--EDUCATION_STATUS_NO, SCHOOL_NAME
ALTER TABLE "RESUME_EDUCATION" 
MODIFY EDUCATION_STATUS_NO NULL;
DELETE FROM SHOP s 
WHERE SHOP_NO=100;
...
JOIN "DOSI" USING (DOSI_NO)
WHERE DONG_NO = 7599;
	SELECT HIRE_NO, HIRE_TITLE, HIRE_CONTENT, TYPE_NAME, HIRE_COUNT,SHOP_NO,
	TO_CHAR(HIRE_END, 'YYYY-MM-DD') HIRE_END,
	DECODE(HIRE_TERM, 1, '하루', 7, '일주일', 
	...
	WHERE HIRE_NO = 83;
	ALTER TABLE APPLY_HIRE ADD APPLY_DATE DATE DEFAULT SYSDATE NOT NULL;
	COMMIT;

SELECT COUNT(*) FROM "HIRE"
		WHERE SYSDATE > HIRE_END ;
	SELECT * FROM "HIRE"
	ORDER BY HIRE_END ASC;
SELECT * FROM RESUME 
...
AND MEMBER_NO =100;
	...
	END;
COMMIT;
ALTER TABLE RESUME ADD RESUME_DEL_FL CHAR(1)
DEFAULT 'N' NOT NULL;
SELECT COUNT(*), MEMBER_NO FROM "RESUME"
GROUP BY MEMBER_NO 
HAVING COUNT(*) >=5;
SELECT * FROM RESUME r 
WHERE MEMBER_NO =100;
DELETE FROM RESUME 
WHERE RESUME_NO IN (8,6);
COMMIT;
DELETE FROM CAREER  ah
WHERE RESUME_NO IN (8,6);
SELECT COUNT(*) FROM HIRE; 
SELECT COUNT(*) FROM (
...
SELECT A.SHOP_NAME, HIRE_TITLE, WORK_START,HIRE_NO,
		...
		WHERE RESUME_NO=30;
		SELECT *, RESUME_NO AS RESUME_NO 
		FROM RESUME_WORK A
		JOIN WORK_TYPE B ON (A.TYPE_NO=B.TYPE_NO)
		WHERE RESUME_NO=30;
		SELECT * FROM "CAREER" WHERE RESUME_NO=12;
		SELECT *
       ...
       JOIN "SIGUNGU" USING (SIGUNGU_NO)
       JOIN "DOSI" USING (DOSI_NO)
       WHERE DONG_NAME LIKE '%성수동1가%'
       AND SIGUNGU_NAME LIKE '%성동구%'
       AND DOSI_NAME LIKE '%서울%';
       SELECT COUNT(*) FROM "DONG";
       SELECT COUNT(*) FROM "SIGUNGU";
       SELECT COUNT(*) FROM "DOSI";
       SELECT HIRE_NO, DONG_NO, HIRE_STATUS, ROAD_ADDRESS 
       FROM "HIRE"
       WHERE HIRE_END >(SELECT SYSDATE FROM "DUAL")
       AND DONG_NO =0
       ORDER BY HIRE_NO;
	SELECT DONG_NAME, DONG_NO, SIGUNGU_NAME FROM DONG 
	JOIN SIGUNGU USING (SIGUNGU_NO)
	WHERE DONG_NAME  LIKE '%인계%'
	AND SIGUNGU_NAME LIKE '%수원%';
	UPDATE HIRE SET DONG_NO=183 WHERE HIRE_NO =147;
	SELECT * FROM HIRE WHERE HIRE_NO =145;
	COMMIT;
	SELECT * FROM LICENSE WHERE RESUME_NO =30;
	UPDATE LICENSE SET LICENSE_FROM = 'ETS' ,
	LICENSE_NAME ='TOEIC', LICENSE_SCORE =990
	WHERE RESUME_NO =30;
	UPDATE RESUME SET INTRODUCE ='누구보다 성실하게 일하겠습니다.' WHERE RESUME_NO=30;